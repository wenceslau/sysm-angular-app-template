    ...
    return request$.pipe(
      ...
      catchError(error => {
        ...
        if (error instanceof HttpErrorResponse && error.status === 0) {
          // This is a generic network error. It can be a CORS issue, a DNS problem,
          // or the server being offline (e.g., ERR_CONNECTION_REFUSED).
          // The browser does not expose the specific cause to JavaScript for security reasons.
          error = {
            error: "Network Error",
            message: "A network error occurred. This could be a CORS issue or the server may be down. " +
